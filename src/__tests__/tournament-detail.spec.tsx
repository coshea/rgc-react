...
function emitCollection(path: string, docs: Array<{ id: string; data: () => any }> ) {
  act(() => {
...
vi.mock('firebase/firestore', async () => {
  return {
    ...
    collection: (_db: any, col: string, id?: string, sub?: string) => id && sub ? `${col}/${id}/${sub}` : `${col}`,
    query: (c: any) => c,
    orderBy: () => ({}),
    onSnapshot: (ref: any, next: any) => {
      listeners[ref] = listeners[ref] || [];
      listeners[ref].push(next);
      return () => { listeners[ref] = listeners[ref].filter((fn) => fn !== next); };
    }
  };
});
...
vi.mock('react-markdown', () => ({ default: (p: any) => <div data-testid="md">{p.children}</div> }));
vi.mock('remark-gfm', () => ({}));

// Mock TournamentEditor lazy import so Suspense fallback resolves immediately
vi.mock('@/components/tournament-editor', () => ({ TournamentEditor: () => <div data-testid="editor">Editor</div> }));
...
const baseTournament = {
  ...
  prizePool: 500,
  winners: [ { place: 1, displayNames: ['Alice'], userIds: ['u1'], prizeAmount: 100 } ],
  tee: 'Blue'
};
...
describe('TournamentDetailPage', () => {
  ...
  it('shows registration open chip and register button when open and user not registered', async () => {
    ...
    emitDoc('tournaments/open1', { ...baseTournament, registrationOpen: true, winners: [] });
  ...
  it('shows registered state when user is part of a team', async () => {
    ...
    emitCollection('tournaments/reg1/registrations', [
      { id: 'r1', data: () => ({ ownerId: 'user1', team: [{ id: 'user1', displayName: 'You' }] }) },
    ]);
  ...
  it('shows closed message when registration closed', async () => {
    ...
    emitDoc('tournaments/closed1', { ...baseTournament, registrationOpen: false });
    await screen.findByText('Club Championship');
  ...
  it('shows only first place winners list', async () => {
    ...
    emitDoc('tournaments/win1', { ...baseTournament, winners: [ { place: 1, displayNames: ['Champ'], userIds: ['c1'], prizeAmount: 50 }, { place: 2, displayNames: ['Runner'], userIds: ['r1'], prizeAmount: 25 } ] });
    await screen.findByText('Club Championship');
  ...
  it('shows admin action buttons when user is admin', async () => {
    ...
    await screen.findByText('Club Championship');
    expect(screen.getByRole('button', { name: /Edit tournament/i })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /Delete tournament/i })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /Export registrations/i })).toBeInTheDocument();
  });
});
