...
vi.mock('@/api/users', () => ({
  getUserProfile: vi.fn(async (_uid: string) => ({ name: 'Test User', email: 't@test.com' })),
  saveUserProfile: vi.fn(async (_uid: string, data: any) => data),
}));
vi.mock('@/api/storage', () => ({
  uploadProfilePicture: vi.fn(async (_uid: string, _file: File) => 'https://storage.test/avatar.png'),
}));
...
describe('useUserProfile', () => {
  ...
  expect(result.current.userProfile?.name).toBe('Test User');
    ...
    await act(async () => {
      await result.current.save({ data: { name: 'New' }, file: new File(['1'], 'f.png') });
    });
